...
class CenternetDataset(Dataset):
    def __init__(self, annotation_lines, input_shape, num_classes, train, center_only=False, center_radius=2):
    ...
    def __getitem__(self, index):
        ...
        #   进行数据增强
        #-------------------------------------------------#
        image, targets  = self.get_random_data(self.annotation_lines[index], self.input_shape, random = self.train, center_only = self.center_only)
        ...
        if self.center_only:
            if len(targets) != 0:
                centers = np.array(targets, dtype=np.float32)
            else:
                centers = np.zeros((0, 3), dtype=np.float32)

            for i in range(len(centers)):
                x, y, cls_id = centers[i]
                cls_id = int(cls_id)

                ct = np.array([x / self.input_shape[1] * self.output_shape[1],
                               y / self.input_shape[0] * self.output_shape[0]], dtype=np.float32)
                ct_int = ct.astype(np.int32)
                if ct_int[0] < 0 or ct_int[0] >= self.output_shape[1] or ct_int[1] < 0 or ct_int[1] >= self.output_shape[0]:
                    continue

                radius = max(0, int(self.center_radius))
                batch_hm[:, :, cls_id] = draw_gaussian(batch_hm[:, :, cls_id], ct_int, radius)
                batch_reg[ct_int[1], ct_int[0]] = ct - ct_int
                batch_reg_mask[ct_int[1], ct_int[0]] = 1
        else:
            if len(targets) != 0:
                boxes = np.array(targets[:, :4],dtype=np.float32)
    ...
    def get_random_data(self, annotation_line, input_shape, jitter=.3, hue=.1, sat=0.7, val=0.4, random=True, center_only=False):
        ...
        if center_only:
            ...
            for item in line[1:]:
                ...
                if len(parts) == 5:
                    left, top, right, bottom, cls_id = parts
                    x = (left + right) / 2.0
                    y = (top + bottom) / 2.0
                elif len(parts) == 3:
                    x, y, cls_id = parts
                    ...
                    raise ValueError("Center-only annotation must be x,y[,cls] or box with cls.")
                centers.append([x, y, cls_id])
        ...
        if not random:
            ...
            if center_only:
                if len(centers) > 0:
                    centers[:, 0] = centers[:, 0]*nw/iw + dx
                    centers[:, 1] = centers[:, 1]*nh/ih + dy
                    centers[:, 0] = np.clip(centers[:, 0], 0, w - 1)
                    centers[:, 1] = np.clip(centers[:, 1], 0, h - 1)
                return image_data, centers
            else:
                if len(box)>0:
